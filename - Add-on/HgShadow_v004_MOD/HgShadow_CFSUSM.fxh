...
{
    // ライト方向のビュー変換行列
	float4x4 LtViewMat = HgShadow_CreateViewMatrix(HgShadow_SMapOrg, OG_LightDirection, float3(0.0f, 1.0f, 0.0f));
	if ( SF_Valid ) {
	LtViewMat = HgShadow_CreateViewMatrix(HgShadow_SMapOrg, HgShadow_LightDirection, float3(0.0f, 1.0f, 0.0f));
	} else {
    LtViewMat = HgShadow_CreateViewMatrix(HgShadow_SMapOrg, OG_LightDirection, float3(0.0f, 1.0f, 0.0f));
	}
