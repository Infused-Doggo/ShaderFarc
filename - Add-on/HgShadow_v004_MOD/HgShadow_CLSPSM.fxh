...
#define SF_Control  "#ShaderFarc_Controller.pmx"
bool SF_Valid  : CONTROLOBJECT < string name = SF_Control; >;

// ライト方向(正規化済み)
float Override : CONTROLOBJECT < string name = SF_Control; string item = "Override";>;
float4x4 IBL_Controller : CONTROLOBJECT < string name = SF_Control; string item = "Chara_Position" ; >;
...
float3 OG_LightDirection  : DIRECTION < string Object = "Light"; >;
static float3 HgShadow_LightDirection = lerp(OG_LightDirection, -Light_Position(IBL_Dir) * float3(1, 1, -1), (int)Override);
...
float4 HgShadow_CLSPSM_GetLightProjPosition(float4 WPos, int mapIndex)
{
	...
	if ( SF_Valid ) {
	cosAng = dot(HgShadow_CameraDirection, HgShadow_LightDirection);
	} else {
    cosAng = dot(HgShadow_CameraDirection, OG_LightDirection);
	}
	...
	if ( SF_Valid ) {
	ltViewMat = HgShadow_CreateViewMatrix(HgShadow_CameraPosition, HgShadow_LightDirection, HgShadow_CameraDirection);
	} else {
    ltViewMat = HgShadow_CreateViewMatrix(HgShadow_CameraPosition, OG_LightDirection, HgShadow_CameraDirection);
	}
