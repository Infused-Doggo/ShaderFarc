...
static float4 g_light_env_stage_specular = Override ? stage_specular : LightAmbient * 1.6;
...
static float4 g_light_env_chara_specular = Override ? chara_specular : LightAmbient * 1.6;

static float4 g_light_chara_dir = float4(-Light_Direction.xyz * float3(1, 1, -1), 1.0);
static float4 g_light_chara_spec = float4(36.81458, 36.81458, 36.81458, 0.00) * (Override ? chara_specular : LightAmbient * 1.6);
static float4 g_light_chara_luce = float4(6.15184, 6.15184, 6.15184, 0.00);
static float4 g_light_chara_back = float4(18.1604, 18.1604, 18.1604, 0.00) * (Override ? chara_specular : LightAmbient * 1.6);
...
#else
static float4 g_material_state_shininess = float4(max(((PMX_Color ? SpecularPower : Shininess) - 16.0f)*(1.0f/112.0f), 0.0f), 0.00, 0.00, 0.00);
#endif
